...
internal static class ResponseMappers
{
    ...
    internal static AdminPlayerSummaryResponse ToAdminSummary(Player player, EconomyService economy)
        => new(
            ...
            player.Account.EmailVerified,
            player.Account.DiscordUsername,
            player.Account.DiscordUserId,
            player.City,
        ...
        IReadOnlyDictionary<string, string>? strikeBlockers = null)
    {
        ...
        return new PlayerProfileResponse(
            ...
            player.Thugs,
            player.Weapons,
            // Everything below here is scouted rather than published, and null means "not known" rather
            ...
            intel.Level >= IntelLevels.Stock ? player.Coke : null,
            intel.Level >= IntelLevels.Morale ? Math.Round(player.HoeHappiness, 2) : null,
            intel.Level >= IntelLevels.Morale ? Math.Round(player.ThugHappiness, 2) : null,
            intel.Level >= IntelLevels.Morale ? AverageMorale(player) : null,
            intel.Level >= IntelLevels.FightingWeight ? ToCombatReadiness(player, options) : null,
            ToCombatStatus(player, nowUtc, viewer, options, recentAttacksMade, recentDefenses, viewerLaneReadyAtUtc, mismatch),
            intel,
            messageBlock is null,
            messageBlock,
    ...
    ///
    /// The rung underfoot and the next one, and no further. The whole ladder used to ship - every rung,
    /// its price and the guns hanging off it - and it read as a spoiler: the shop opened by naming every
    /// weapon and every threshold in the game to somebody who had not bought a shotgun yet. What is left
    /// is what a player is and what is next. A locked row on the shelf still names the rung that opens
    /// it, so nothing anybody is actually reaching for went quiet with it.
    /// </summary>
    ...
    internal static HideoutResponse ToHideoutResponse(Player player, HideoutService hideouts, DateTime nowUtc, GameOptions options, WorkshopCraft? craft = null)
    {
        ...
        return new HideoutResponse(
            ...
            player.Hideout?.CokeLabRunning ?? true,
            player.Hideout?.WeedLabAutoSell ?? false,
            player.Hideout?.CokeLabAutoSell ?? false,
            options.Hideout.MinLabLevelForAutoSell,
