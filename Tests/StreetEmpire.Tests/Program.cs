...
static void TheLabsReportOncePerAbsence()
{
    ...
    var snapshot = Snapshot(options);
    using var db = new GameDbContext(new DbContextOptionsBuilder<GameDbContext>()
        .UseInMemoryDatabase($"lab-report-{Guid.NewGuid()}")
        .Options);

    var (player, _) = AccountSetup.NewPlayer(
        new PlayerAccount { Username = "chem" }, "Chem", "Detroit", options, CreateRoster(options));
    player.Hideout!.WeedLabLevel = 3;
    player.Hideout.WeedLabRunning = true;
    player.Hideout.WeedLabAutoSell = true;
    db.Players.Add(player);
    db.SaveChanges();
    ...
    player.Hideout.LabsCollectedAtUtc = start;

    var labRows = () => db.ActionLogs.Where(x => x.PlayerId == player.Id && x.Action == "LAB").ToList();

    // Four hours pass an hour at a time, the way they do for somebody sitting at the screen. The money
    // lands every hour - that is untouched - but it is all one report.
    for (var hour = 1; hour <= 4; hour++)
    {
        clock.AdvanceAsync(player, start.AddHours(hour), db).GetAwaiter().GetResult();
        db.SaveChanges();
    }

    var afterFour = labRows();
    AssertEqual(1, afterFour.Count);
    AssertEqual(4, player.Hideout.PendingLabHours);
    var runEarnings = afterFour[0].CashDelta;
    AssertTrue(runEarnings > 0, "the row should carry what the whole run earned");

    // Reading it closes the run. The next hour is a new absence and so a new row, and the one they
    // already read is left where it was rather than growing under them.
    player.CombatAlertsSeenAtUtc = start.AddHours(4).AddMinutes(1);
    clock.AdvanceAsync(player, start.AddHours(5), db).GetAwaiter().GetResult();
    db.SaveChanges();

    var afterReading = labRows().OrderBy(x => x.CreatedAtUtc).ToList();
    AssertEqual(2, afterReading.Count);
    AssertEqual(runEarnings, afterReading[0].CashDelta);
    AssertTrue(afterReading[1].CashDelta > 0 && afterReading[1].CashDelta < runEarnings,
        "the new run starts from nothing rather than carrying the old one forward");
    AssertEqual(1, player.Hideout.PendingLabHours);

    // And an unread run keeps growing into the same row rather than adding another.
    clock.AdvanceAsync(player, start.AddHours(6), db).GetAwaiter().GetResult();
    db.SaveChanges();
    AssertEqual(2, labRows().Count);
    AssertEqual(2, player.Hideout.PendingLabHours);
}
